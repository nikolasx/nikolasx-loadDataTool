...
{
    public class LoadRelocationSlope
    {

        public string InsertData(string flowFilePath, string comFilePath)
        {
            //DataTable slopeTable = AccessHelper.GetDataTableByAccessFile(flowFilePath, "斜坡主表");

            //TODO 临时修改
            DataTable slopeTable = SqlHelper.GetDataTable("select * from [dbo].[TheRelocationSlope]");
            ...
            for (int i = 0; i < slopeTable.Rows.Count; i++)
            {
                try
                {
                    RelocationSlope slope = GetRelocationSlope(slopeTable.Rows[i], slopeTable.Columns);
        ...
        public RelocationSlope GetRelocationSlope(DataRow row, DataColumnCollection columns)
        {
            RelocationSlope slope = new RelocationSlope();
