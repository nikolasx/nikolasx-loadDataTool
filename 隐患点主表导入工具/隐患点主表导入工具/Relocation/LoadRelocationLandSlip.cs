...
{
    public class LoadRelocationLandSlip
    {

        public  string InsertData(string landFilePath, string comFilePath)
        {
            //DataTable landTable = AccessHelper.GetDataTableByAccessFile(landFilePath, "崩塌主表");

            //TODO 临时修改
            DataTable landTable = SqlHelper.GetDataTable("select * from [dbo].[TheRelocationCollapse]");
            ...
            for (int i = 0; i < landTable.Rows.Count; i++)
            {
                try
                {
                    RelocationLandSlip landSlip = GetRelocationLandSlip(landTable.Rows[i], landTable.Columns);
        ...
        public RelocationLandSlip GetRelocationLandSlip(DataRow row, DataColumnCollection columns)
        {
            RelocationLandSlip landSlip = new RelocationLandSlip();
            ...
            landSlip.威胁财产 = Convert.ToInt32(row["威胁财产"].ToString() == "" ? "0" : row["威胁财产"].ToString());
