...
{
    class LoadRelocationLandFracture
    {
        ...
        public  string InsertData(string landFilePath, string comFilePath)
        {
            ...
            //TODO 临时修改
            DataTable landTable = SqlHelper.GetDataTable("select * from [dbo].[TheRelocationGroundFissure]");
            ...
            for (int i = 0; i < landTable.Rows.Count; i++)
            {
                try
                {
                    RelocationLandFracture landFracture = GetRelocationLandFracture(landTable.Rows[i], landTable.Columns);
        ...
        public RelocationLandFracture GetRelocationLandFracture(DataRow row, DataColumnCollection columns)
        {
            RelocationLandFracture landFracture = new RelocationLandFracture();
