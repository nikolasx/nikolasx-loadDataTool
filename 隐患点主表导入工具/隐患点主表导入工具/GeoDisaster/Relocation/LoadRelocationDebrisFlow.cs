...
{
    public class LoadRelocationDebrisFlow
    {

        /// <summary>
        /// 插入移民搬迁泥石流数据
        /// </summary>
        /// <param name="flowFilePath"></param>
        /// <param name="comFilePath"></param>
        /// <returns></returns>
        public string InsertData(string flowFilePath, string comFilePath)
        {

            //DataTable flowTable = AccessHelper.GetDataTableByAccessFile(flowFilePath, "泥石流主表");

            //TODO 临时修改
            DataTable flowTable = SqlHelper.GetDataTable("select * from [dbo].[TheRelocationDebrisFlow]"); 
            ...
            for (int i = 0; i < flowTable.Rows.Count; i++)
            {
                try
                {
                    RelocationDebrisFlow flow = this.GetRelocationDebrisFlow(flowTable.Rows[i], flowTable.Columns);
                    string uId = flowTable.Rows[i]["统一编号"].ToString();
                    DataRow[] rows = comTable.Select("统一编号=" + uId);
                    if (rows.Length > 1 || rows.Length < 1)
                        throw new Exception(@"不存在综合表或综合表不唯一");

                    //使用AutoMapper将十万调查的类型转化为移民搬迁的类型
        ...
        public RelocationDebrisFlow GetRelocationDebrisFlow(DataRow row, DataColumnCollection columns)
        {
            RelocationDebrisFlow flow = new RelocationDebrisFlow();
