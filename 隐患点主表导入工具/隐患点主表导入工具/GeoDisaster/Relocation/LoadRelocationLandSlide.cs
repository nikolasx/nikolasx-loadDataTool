...
{
    public class LoadRelocationLandSlide
    {


        public string InsertData(string landFilePath, string comFilePath)
        {
            //DataTable landTable = AccessHelper.GetDataTableByAccessFile(landFilePath, "滑坡主表");

            //TODO 临时修改
            DataTable landTable = SqlHelper.GetDataTable("select * from [dbo].[TheRelocationLandslide]");
            ...
            for (int i = 0; i < landTable.Rows.Count; i++)
            {
                try
                {
                    RelocationLandSlide landSlide = GetRelocationLandSlide(landTable.Rows[i], landTable.Columns);
        ...
        public RelocationLandSlide GetRelocationLandSlide(DataRow row, DataColumnCollection columns)
        {
            RelocationLandSlide landslide = new RelocationLandSlide();
