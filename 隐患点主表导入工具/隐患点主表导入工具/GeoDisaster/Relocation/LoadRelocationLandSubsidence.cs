...
{
    public class LoadRelocationLandSubsidence
    {
        public string InsertData(string landFilePath, string comFilePath)
        {
            //DataTable landTable = AccessHelper.GetDataTableByAccessFile(landFilePath, "地面沉降主表");

            //TODO 临时修改
            DataTable landTable = SqlHelper.GetDataTable("select * from [dbo].[TheRelocationGroundSubsidence]");
            ...
            for (int i = 0; i < landTable.Rows.Count; i++)
            {
                try
                {
                    RelocationLandSubsidence landSubsidence = GetRelocationLandSubsidence(landTable.Rows[i], landTable.Columns);
        ...
        public RelocationLandSubsidence GetRelocationLandSubsidence(DataRow row, DataColumnCollection columns)
        {
            RelocationLandSubsidence landSubsidence = new RelocationLandSubsidence();
